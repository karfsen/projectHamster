...
function closeDelGoal(){
  document.getElementById('deletecharGoal').style.display='none';
    ...
    if(this.readyState==4&&this.status==200){
      ...
      if(this.responseText!="[]"){
        ...
        array.forEach(element=>{
          document.getElementById("goals").appendChild(createDisBox(element));
  ...
  xhr2.onreadystatechange=function(){
    if(this.readyState==4&&this.status==200){
      ...
      if(this.responseText!="[]"){
        array.forEach(element=>{
          document.getElementById("nogoals").style.display="none";
          document.getElementById("goals").appendChild(createCharBox(element));
          let done=Math.round(((element.amount-element.remaining)*100)/element.amount*100)/100;
          let todo=100-done;
          let chart=drawChart(element.id,done,todo);
          document.getElementById(element.id).innerHTML=chart;
        });
...
function createCharBox(element){
  console.log(element);
  let WhiteBox=document.createElement("div");
  WhiteBox.className="WhiteBox";
  ...
  let goalName=document.createElement("div");
  goalName.className="goalName";
  goalName.innerHTML="CHARGE GOAL";
  ...
  div1_1.className="CityNameLabel";
  div1_1.id="from";
  div1_1.innerHTML=element.device;
  ...
  let div6=document.createElement("div");
  div6.className="disGoalQuote";
  div6.innerHTML="Baterry balance";
  let div7=document.createElement("div");
  div7.className="timeDiv";
  div7.id="distanceDiv";
  div7.innerHTML=(element.amount-element.remaining)+" mAh of "+element.amount+" mAh";
  ...
  div9.className="timeDiv";
  div9.id="distanceDiv";
  div9.innerHTML=((element.amount-element.remaining)*4000)+" of "+element.amount*4000;
  let id=element.id;
  let graphSide=document.createElement("div");
  graphSide.id=id+"a";
  graphSide.innerHTML='<svg onclick="oncharXclick('+id+')" xmlns="http://www.w3.org/2000/svg" class="delGoal" width="24px" height="24px" viewBox="0 0 48 48"><path fill="#0700FF" fill-rule="evenodd" d="M22.586 25L9.15 11.565l1.414-1.414L24 23.586 37.435 10.15l1.414 1.414L25.414 25 38.85 38.435l-1.414 1.414L24 26.414 10.565 39.85l-1.414-1.414L22.586 25z"/></svg>';
  let graph_container=document.createElement("canvas");
  graph_container.style="width:154px; height:154px; margin:16px";
  graph_container.id=id;
...
oncharXclick=(id)=>{
  ...
  document.getElementById("delModal").style.display="block";
  document.getElementsByClassName("Group")[0].style.display="none";
  document.getElementById("deletecharGoal").style.display="block";
}
...
removecharGoal=()=>{
  let data={id:pid};
  console.log(JSON.stringify(data));
  fetch('http://itsovy.sk:1206/deletechargoal',{
    method: 'POST', 
    mode: 'cors', 
    cache: 'no-cache', 
    credentials: 'same-origin', 
...
char3step=function(){
  ...
  if(chargoalSelected==1){
    ...
    document.getElementById("baterryCapacity").innerHTML="12960 mAh";
  }
}
...
function submitChargeGoal(){
  ...
  if(chargoalSelected==1){
    ...
    xhr.send(JSON.stringify({device:"iPad",amount:8827}));  
    document.getElementById("ipadChargeSVG").style.display="none"; 
  }
  else if(chargoalSelected==3){
    xhr.send(JSON.stringify({device:"MacBook Pro",amount:12960}));   
    document.getElementById("macChargeSVG").style.display="none";
  }
